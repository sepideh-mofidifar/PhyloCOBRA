...
class Community(cobra.Model):
    ...
    def __init__(
        ...
        if model_db is not None:
            ...
            phylogenetic_distance = data.phylogenetic_distance
            similarity_dist = pd.DataFrame(phylogenetic_distance[id])
#########################################################

            if 'genus' in similarity_dist.columns:
                similarity_dist = similarity_dist.set_index('genus')
            ...
            visited = set()
            groups = []
            indices_to_remove = set()  # For collecting subset indices

            # Grouping based on similarity
            for i in similarity_dist.index:
                if i in visited:
                    continue
                group = set([i])
                visited.add(i)
                for j in similarity_dist.columns:
                    if j in visited or i == j:
                        continue
                    # Skip symmetric duplicates
                    if similarity_dist.columns.get_loc(j) <= similarity_dist.index.get_loc(i):
                        continue
                    if similarity_dist.loc[i, j] > threshold:
                        group.add(j)
                        visited.add(j)
                groups.append(group)

            # Combine rows based on groups
            processed_rows = []

            for group in groups:
                subset = merged[merged['genus'].str.split().str[0].isin(group)]
                if not subset.empty:
                    combined_row = {
                        'sample_id': subset['sample_id'].iloc[0],
                        'abundance': subset['abundance'].sum(),
                        'genus': "+".join(subset['genus'].unique()),
                        'id': "+".join(subset['genus'].unique()),
                        'file': list(subset['file']),  # Flatten list of files  # Flatten nested list
                        'family': "+".join(subset['family'].unique())
                    }
                    processed_rows.append(combined_row)

                    # Mark these indices for deletion ONLY IF the group has more than one genus
                    if len(group) > 1:
                        indices_to_remove.update(subset.index)

            # Remove collected indices once, after loop
            merged = merged[~merged.index.isin(indices_to_remove)]

            # Add processed rows
            processed_df = pd.DataFrame(processed_rows)
            merged = pd.concat([merged, processed_df], ignore_index=True)

            # Optional: drop duplicate genus entries based on full genus name
            merged = merged.loc[~merged['genus'].duplicated(keep='first')]
